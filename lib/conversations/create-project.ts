...
export async function createProject(
  ...
  // 3. Twitter ID or URL
  await ctx.reply(
    `<code>[3/${steps}]</code>\nDo you have a Twitter handle or a Twitter URL? Please provide it. Type "no" if you don't want to add it.`,
    { parse_mode: "HTML" },
  );
  const twitterInput = await conversation.form.text(async (ctx) => {
    await ctx.reply(
      "Please provide a valid input (Twitter handle/URL or 'no').",
    );
  });
  ...
  if (twitterInput.startsWith("http")) {
    ...
    if (!twitterId) {
      await ctx.reply(
        "⚠️ The provided Twitter URL is invalid. Please try again or type 'no'.",
      );
      return; // Stops further execution if invalid input
    }
  }
  const twitterLink =
    twitterId !== "no" ? `https://twitter.com/${twitterId}` : null;

  // 4. GitHub ID or URL
  await ctx.reply(
    `<code>[4/${steps}]</code>\nDo you have a GitHub ID or GitHub URL? Please provide it. Type "no" to skip.`,
    { parse_mode: "HTML" },
  );
  const githubInput = await conversation.form.text(async (ctx) => {
    await ctx.reply(
      "Please provide a valid input (GitHub handle/URL or 'no').",
    );
  });
  ...
  if (githubInput.startsWith("http")) {
    ...
    if (!githubId) {
      await ctx.reply(
        "⚠️ The provided GitHub URL is invalid. Please try again or type 'no'.",
      );
      return; // Stops further execution if invalid input
    }
  }
  const githubLink =
    githubId !== "no" ? `https://github.com/${githubId}` : null;

  // 5. Add an image
  await ctx.reply(
    `<code>[5/${steps}]</code>\nFab! Let's add an image for the project. Send a photo or an image of the project.`,
    { parse_mode: "HTML" },
  );
  ...
  try {
    ...
    const { data: imageRes, error } = await supabase.storage
      .from("gmon.link")
  ...
  await ctx.api.editMessageText(
    ...
    `🎉 <b>Project created successfully!</b> Here's the link to the project: <a href="gmon.link/${slug}">gmon.link/${slug}</a>`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: "Add a Link", callback_data: "create-link" }],
        ],
      },
    },
  );
  ...
  await ctx.api.sendMessage(
    botSettings.alertsChannelId,
    `${user.username} created a new project. <a href="gmon.link/${slug}">gmon.link/${slug}</a>`,
